...
bool MidasFile::Open(const char *filename){
  std::cout << "Reading midas file : " << filename << std::endl;
  ...
  if (strncmp(filename, "ssh://", 6) == 0)
    ...
    if (s == NULL)
      {
      fLastErrno = -1;
      fLastError = "MidasFile::Open: Invalid ssh:// URI. Should be: ssh://user@host/file/path/...";
      return false;
      }
  ...
  if (pipe.length() > 0)
    {
    fprintf(stderr,"MidasFile::Open: Reading from pipe: %s\n", pipe.c_str());
...
bool MidasFile::Read(MidasEventFragment* midasEvent){
  /// \param [in] midasEvent Pointer to an empty MidasEvent
