//
//  MidasEventFragment.cxx.
//
//  $Id: MidasEventFragment.cxx 70 2009-06-20 02:12:14Z olchansk $
//
...
MidasEventFragment::MidasEventFragment(){
  fData = NULL;
...
void MidasEventFragment::Copy(const MidasEventFragment& rhs){
...
MidasEventFragment::MidasEventFragment(const MidasEventFragment &rhs){
  Copy(rhs);
}
...
MidasEventFragment& MidasEventFragment::operator=(const MidasEventFragment &rhs){
  if (&rhs != this)
    Clear();
...
void MidasEventFragment::SetData(uint32_t size, char* data){
...
uint16_t MidasEventFragment::GetEventId() const{
  return fEventHeader.fEventId;
}

uint16_t MidasEventFragment::GetTriggerMask() const{
  return fEventHeader.fTriggerMask;
}

uint32_t MidasEventFragment::GetSerialNumber() const{
  return fEventHeader.fSerialNumber;
}

uint32_t MidasEventFragment::GetTimeStamp() const{
  return fEventHeader.fTimeStamp;
}

uint32_t MidasEventFragment::GetDataSize() const{
  return fEventHeader.fDataSize;
}

char* MidasEventFragment::GetData(){
  if (!fData){    
    AllocateData();
  }
  return fData;
...
EventHeader_t * MidasEventFragment::GetEventHeader(){
  return &fEventHeader;
}

bool MidasEventFragment::IsGoodSize() const{
  return fEventHeader.fDataSize > 0 && fEventHeader.fDataSize <= 500 * 1024 * 1024;
}

bool MidasEventFragment::IsBank32() const{
  return ((BankHeader_t *)fData)->fFlags & (1<<4);
}

int MidasEventFragment::LocateBank(const void *unused, const char *name, void **pdata) const{
  /// See FindBank()
...
int MidasEventFragment::FindBank(const char* name, int *bklen, int *bktype, void **pdata) const{
  /// Find a data bank.
...
void MidasEventFragment::Print(const char *option) const{
  ...
  if ((fEventHeader.fEventId & 0xffff) == 0x8000)
    ...
    printf("End of run %d\n", fEventHeader.fSerialNumber);
      ...
      {
    printf("MidasEventFragment::Print: Use SetBankList() before Print() to print bank data\n");
      }
  else
      {
    printf("Banks: %s\n", fBankList);
    ...
    for (int i = 0; i < fBanksN * 4; i += 4)
      ...
      if (option[0] == 'a' && found)
        switch (bankType)
          ...
          break;
            default:
          printf("MidasEventFragment::Print: Do not know how to print bank of type %d\n", bankType);
          break;
          }
        }
      }
...
void MidasEventFragment::AllocateData(){
  assert(!fAllocatedByUs);
  assert(IsGoodSize());
  fData = (char*)malloc(fEventHeader.fDataSize);
  assert(fData);
...
const char* MidasEventFragment::GetBankList() const{
  return fBankList;
}

int MidasEventFragment::SetBankList(){
...
int MidasEventFragment::IterateBank(Bank_t **pbk, char **pdata) const{
...
int MidasEventFragment::IterateBank32(Bank32_t **pbk, char **pdata) const{
  /// See IterateBank()
...
void MidasEventFragment::SwapBytesEventHeader(){
...
int MidasEventFragment::SwapBytes(bool force){  
  BankHeader_t *pbh;
  Bank_t *pbk;
  Bank32_t *pbk32;
  void *pdata;
